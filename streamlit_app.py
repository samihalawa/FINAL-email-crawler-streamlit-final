...
from sqlalchemy import func, create_engine, Column, BigInteger, Text, DateTime, ForeignKey, Boolean, JSON, Float, Index, UniqueConstraint
...
from sqlalchemy import select, text, distinct, and_
...
# Use add_job_safely instead of scheduler.add_job throughout the code
...
class Campaign(Base):
    ...
    campaign_leads = relationship("CampaignLead", backref="campaign")
...
class Lead(Base):
    ...
    __table_args__ = (UniqueConstraint('id', name='uq_lead_id'),)
...
def get_background_state():
    with safe_db_session() as session:
        try:
            state = session.query(BackgroundProcessState).filter_by(campaign_id=get_active_campaign_id()).first()
            if not state:
                state = BackgroundProcessState(campaign_id=get_active_campaign_id())
                session.add(state)
                session.commit()
            return {
            ...
            logging.error(f"Error getting background state: {str(e)}")
            return {
                'is_running': False,
                'last_run': None,
                'current_term': None,
...
def update_background_state(session, **kwargs):
    state = session.query(BackgroundProcessState).filter_by(campaign_id=get_active_campaign_id()).first()
    if state:
...
def ai_automation_loop(session, log_container, leads_container):
    ...
    while st.session_state.get('automation_status', False) and iteration < max_iterations:
        try:
            ...
            iteration += 1
            time.sleep(3600)  # Sleep for an hour between iterations
        except Exception as e:
