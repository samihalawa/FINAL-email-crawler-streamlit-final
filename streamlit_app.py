...
class CampaignLead(Base):
    ...
    search_term_id = Column(BigInteger, ForeignKey('search_terms.id'))  # Add this line
    status = Column(Text)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    lead = relationship("Lead", back_populates="campaign_leads")
    campaign = relationship("Campaign", back_populates="campaign_leads")
    search_term = relationship("SearchTerm", back_populates="campaign_leads")  # Add this line
...
class SearchTerm(Base):
    ...
    campaign_leads = relationship("CampaignLead", back_populates="search_term")  # Add this line
...
def settings_page():
    ...
    with db_session() as session:
        ...
        with st.form("general_settings_form"):
            ...
            openai_model = st.text_input("OpenAI Model", value=general_settings.value.get('openai_model', 'gpt-4-0125-preview'))
        ...
        for setting in email_settings:
            with st.expander(f"{setting.name} ({setting.email})"):
                ...
                if st.button(f"Delete {setting.name}", key=f"delete_{setting.id}"): session.delete(setting); st.success(f"Deleted {setting.name}"); st.rerun()
        ...
        with st.form("email_setting_form"):
            ...
            if st.form_submit_button("Save Email Setting"):
                setting_data = {k: v for k, v in locals().items() if k in ['name', 'email', 'provider', 'smtp_server', 'smtp_port', 'smtp_username', 'smtp_password', 'aws_access_key_id', 'aws_secret_access_key', 'aws_region']}
                if edit_setting: 
                    for k, v in setting_data.items(): setattr(edit_setting, k, v)
                else:
...
# Update the send_email_ses function
...
# Update the fetch_leads_with_sources function
...
def manual_search_page():
    ...
    if st.button("Search"):
        ...
        for i, term in enumerate(search_terms):
            ...
            with db_session() as session:
                term_results = manual_search(session, [term], num_results, ignore_previously_fetched, optimize_english, optimize_spanish, shuffle_keywords_option, language)
...
def perform_quick_scan(session):
    with st.spinner("Performing quick scan..."):
        ...
        res = manual_search(session, [term.term for term in terms], 10, True, False, False, True, "EN")
...
def bulk_send_emails(session, template_id, from_email, reply_to, leads, progress_bar=None, status_text=None, results=None):
...
def view_campaign_logs():
    ...
    with db_session() as session:
        campaigns = session.query(EmailCampaign.campaign_id, EmailCampaign.sent_at).distinct().order_by(EmailCampaign.sent_at.desc()).all()
        campaign_options = [f"{c.campaign_id}: {c.sent_at.strftime('%Y-%m-%d %H:%M:%S')}" for c in campaigns]
        selected_campaign = st.selectbox("Select Campaign", options=campaign_options, format_func=lambda x: x.split(":")[1].strip())
...
def bulk_send_page():
    ...
    with db_session() as session:
        ...
        with col2:
            send_option = st.radio("Send to:", ["All Leads", "Not Previously Contacted", "Specific Email", "Leads from Chosen Search Terms", "Leads from Search Term Groups"])
            specific_email = None
            selected_terms = None
            if send_option == "Specific Email":
                ...
                selected_terms = [term.split(" (")[0] for term in selected_terms]  # Extract just the term without the lead count
        ...
        total_leads = len(leads)
        st.info(f"Total leads to be contacted: {total_leads}")
        ...
        if st.button("Send Emails", type="primary"):
            ...
            status_text = st.empty()
            results = []
            bulk_send_emails(session, template_id, from_email, reply_to, leads, progress_bar, status_text, results)
            st.success(f"Emails sent successfully to {len(results)} leads.")
...
def bulk_send_page():
    ...
    with db_session() as session:
        ...
        if st.button("Send Emails", type="primary"):
            ...
            status_text = st.empty()
            results = []
            bulk_send_emails(session, template_id, from_email, reply_to, contactable_leads, progress_bar, status_text, results)
            st.success(f"Emails sent successfully to {len(results)} leads.")
...
def perform_quick_scan(session):
    return {"new_leads": len(manual_search(session, fetch_search_terms_with_lead_count(session)[:3], 10, st.empty(), st.empty()))}
...
def automation_control_panel_page():
    ...
    if st.button("Search"):
        search_terms = get_search_terms(session)
        if not search_terms:
                ...
                if enable_email_sending:
                    ...
                    for result in term_results['results']:
                        ...
                        try:
                            ...
                            logging.error(f"Error sending email to {result['Email']}: {str(e)}")
                            emails_sent.append(f"❌ {result['Email']} (Error: {str(e)})")
                            status_text.text(f"Error sending email to: {result['Email']}")
...
def perform_web_search(query, num_results):
    return [{'email': f'example{i}@example.com', 'url': f'https://example{i}.com', 'company': f'Company {i}'} for i in range(num_results)]

def is_lead_exists(session, email):
    return session.query(Lead).filter_by(email=email).first() is not None
...
def main():
    ...
    with st.sidebar:
        selected = option_menu(
            ...
            icons=["search", "send", "people", "key", "envelope", "folder", "book", "robot", "gear", "list-check", "gear"],
            menu_icon="cast",
            default_index=0
        )
