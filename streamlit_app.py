...
def manual_search_page():
        ...
        with db_session() as session:
            ...
            with col3:
                template = st.selectbox(
                    ...
                    format_func=lambda t: f"{t.id}: {t.template_name}",
    ...
    if st.button("Search"):
        ...
        for i, term in enumerate(search_terms):
            ...
            with db_session() as session:
                term_results = manual_search(session, [term], num_results, ignore_previously_fetched, optimize_english, optimize_spanish, shuffle_keywords_option, language, enable_email_sending, log_container, from_email, reply_to, template)
...
def bulk_send_emails(session, template_id, from_email, reply_to, leads, progress_bar=None, status_text=None, results=None, log_container=None):
    ...
    for index, lead in enumerate(leads):
        try:
            ...
            save_email_campaign(session, lead['Email'], template.id, status, datetime.utcnow(), template.subject, message_id, wrapped_content)
            ...
            save_email_campaign(session, lead['Email'], template.id, 'failed', datetime.utcnow(), template.subject, f"error-{uuid.uuid4()}", template.body_content)
...
def get_email_preview(session, template, from_email, reply_to):
    ...
    with db_session() as session:
        ...
        with col2:
            email_setting = st.selectbox(
                "From Email",
                options=email_settings,
                format_func=lambda s: f"{s.name} ({s.email})"
            )
            if email_setting:
                from_email = email_setting.email
        ...
        if send_option == "Specific Email":
            ...
            selected_groups = st.multiselect(
                ...
                options=groups,
                format_func=lambda g: f"{g.id}: {g.name}"
            )
            if selected_groups:
        ...
        st.components.v1.html(get_email_preview(session, template, from_email, reply_to), height=600, scrolling=True)

        leads = fetch_leads(session, template.id, send_option, specific_email, selected_terms, exclude_previously_contacted)
        total_leads = len(leads)
        ...
        if st.button("Send Emails", type="primary"):
            ...
            results = []
            log_container = st.empty()
            logs, sent_count = bulk_send_emails(session, template, from_email, reply_to, contactable_leads, progress_bar, status_text, results, log_container)
