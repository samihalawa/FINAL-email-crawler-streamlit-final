...
def send_email_ses(session, from_email, to_email, subject, body, charset='UTF-8', reply_to=None, ses_client=None):
    try:
        ...
        if not email_settings:
            logger.error(f"No email settings found for {from_email}")  # Use logger instead of logging
            return None, None

        if not all([to_email, subject, body]):
            logger.error("Missing required email parameters")  # Use logger instead of logging
            return None, None
            ...
            try:
                ...
                try:
                    response = client.chat.completions.create(
                        model=general_settings.value.get('openai_model', "Qwen/Qwen2.5-72B-Instruct"),
                        messages=messages,
                ...
                log_ai_request(session, function_name, messages, error_msg, lead_id, email_campaign_id, model_used)
                raise ValueError(error_msg)
    except Exception as e:
...
def manual_search_page():
    ...
    search_tab, processes_tab = st.tabs(["New Search", "Running Processes"])
    
    with search_tab:
        try:
            with db_session() as session:  # Add proper session handling
