...
def bulk_send_page():
    ...
    try:
        ...
        st.error("Email sending timed out. Please try a smaller batch size.")
...
def background_search_worker(message_queue):
    ...
    while True:
        ...
        if process.memory_percent() > 80:  # Memory usage > 80%
            logging.warning("High memory usage detected, pausing background process")
            time.sleep(60)  # Cool down period
            continue
            
        if process.cpu_percent() > 80:  # CPU usage > 80%
            logging.warning("High CPU usage detected, pausing background process")
