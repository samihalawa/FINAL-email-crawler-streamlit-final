...
class BackgroundTask(Base):
    ...
    id = Column(BigInteger, primary_key=True)
    task_type = Column(Text, nullable=False)  # e.g., 'manual_search'
    status = Column(Text, nullable=False)  # 'pending', 'running', 'completed', 'failed'
    settings = Column(JSON, nullable=False)  # Task-specific settings
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
class TaskLog(Base):
    ...
    id = Column(BigInteger, primary_key=True)
    task_id = Column(BigInteger, ForeignKey('background_tasks.id', ondelete='CASCADE'))
    message = Column(Text, nullable=False)
    level = Column(Text, default='info')  # 'info', 'warning', 'error'
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    metadata = Column(JSON)  # Additional context about the log entry
