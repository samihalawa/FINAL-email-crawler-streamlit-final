    ...
    if enable_email_sending:
        ...
        with col4:
            ...
            if email_setting_option:
                ...
                reply_to = st.text_input("Reply To", value=email_setting_option['email'])
            else:
...
def bulk_send_page():
    ...
    with db_session() as session:
        ...
        leads = fetch_leads(session)
        if leads.empty:
            st.error("No leads available. Please gather leads first.")
            return

    col1, col2 = st.columns(2)
    with col1:
        template_option = st.selectbox("Email template", options=email_templates, format_func=lambda x: x.split(":")[1].strip())
    with col2:
        email_setting_option = st.selectbox("From Email", options=email_settings, format_func=lambda x: f"{x['name']} ({x['email']})")
        if email_setting_option:
            from_email = email_setting_option['email']
    ...
    send_option = st.selectbox("Send to", ["All Leads", "Specific Email"], index=0)
    specific_email = st.text_input("Specific Email", placeholder="Enter email address") if send_option == "Specific Email" else None
    selected_terms = st.multiselect("Filter by Search Terms", options=fetch_search_terms(session)) if send_option == "All Leads" else None
    exclude_previously_contacted = st.checkbox("Exclude Previously Contacted Leads", value=True) if send_option == "All Leads" else False

    def initiate_bulk_send():
        if not template_option:
            st.error("Please select an email template.")
            return
        if not from_email:
            st.error("Please select a from email.")
            return
        ...
        if MESSAGE_QUEUE:
            MESSAGE_QUEUE.put({
                ...
                'params': {
                    ...
                    'exclude_previously_contacted': exclude_previously_contacted
                }
            })
            st.session_state.bulk_send_status = "pending"
        else:
            st.error("Background process not initialized.")

    st.button("Send Emails", on_click=initiate_bulk_send)

    if st.session_state.get('bulk_send_status') == "pending":
        st.info("Bulk send is pending...")
    elif st.session_state.get('bulk_send_status') == "running":
        st.info(f"Bulk sending... ({st.session_state.bulk_send_progress:.0%})")
        st.progress(st.session_state.bulk_send_progress)
        if st.session_state.bulk_send_logs:
            st.text_area("Logs", "\n".join(st.session_state.bulk_send_logs), height=200)
    elif st.session_state.get('bulk_send_status') == "completed":
        st.success(f"Bulk send completed! Sent {st.session_state.bulk_emails_sent} emails.")
        if st.session_state.bulk_send_logs:
            st.text_area("Logs", "\n".join(st.session_state.bulk_send_logs), height=200)
    elif st.session_state.get('bulk_send_status') == "failed":
        st.error("Bulk send failed. Check logs for details.")
        if st.session_state.bulk_send_logs:
            st.text_area("Logs", "\n".join(st.session_state.bulk_send_logs), height=200)
...
def start_background_process():
    ...
    if SEARCH_PROCESS is None or not SEARCH_PROCESS.is_alive():
        MESSAGE_QUEUE = multiprocessing.Queue()
        SEARCH_PROCESS = multiprocessing.Process(target=background_search_worker, args=(MESSAGE_QUEUE,))
        ...
        if 'total_terms' not in st.session_state:
            st.session_state.total_terms = 0
...
def background_search_worker(message_queue):
    while True:
        if not message_queue.empty():
            ...
            if message == "STOP":
                break
            elif isinstance(message, dict) and message.get('type') == 'search':
                ...
                try:
                    with db_session() as session:
                        ...
                        st.session_state.search_progress = 0
                        st.session_state.current_term = ""
                        st.session_state.total_terms = len(search_params['terms'])
                        for i, term in enumerate(search_params['terms']):
                            st.session_state.current_term = term
                            message_queue.put({'type': 'log', 'message': f"Searching for '{term}'"})
                            results = manual_search(
                                session,
                                [term],
                                search_params['num_results'],
                                search_params['ignore_previously_fetched'],
                                search_params['optimize_english'],
                                search_params['optimize_spanish'],
                                search_params['shuffle_keywords_option'],
                                search_params['language'],
                                search_params['enable_email_sending'],
                                None,
                                search_params.get('from_email'),
                                search_params.get('reply_to'),
                                search_params.get('email_template'),
                            )
                            st.session_state.search_results.extend(results['results'])
                            st.session_state.search_progress = (i + 1) / len(search_params['terms'])
                            message_queue.put({'type': 'log', 'message': f"Found {len(results['results'])} leads for '{term}'"})
                            
                            if search_params['enable_email_sending']:
                                template = session.query(EmailTemplate).filter_by(id=int(search_params['email_template'].split(":")[0])).first()
                                if template:
                                    for result in results['results']:
                                        if not result or 'Email' not in result or not is_valid_email(result['Email']):
                                            message_queue.put({'type': 'log', 'message': f"Skipping invalid result or email: {result.get('Email') if result else 'None'"})
                                            continue
                                        wrapped_content = wrap_email_body(template.body_content)
                                        response, tracking_id = send_email_ses(session, search_params['from_email'], result['Email'], template.subject, wrapped_content, reply_to=search_params['reply_to'])
                                        if response:
                                            ...
                                            message_queue.put({'type': 'log', 'message': f"Email sent to: {result['Email']}"})
                                        else:
                                            save_email_campaign(session, result['Email'], template.id, 'failed', datetime.utcnow(), template.subject, None, template.body_content)
                                            message_queue.put({'type': 'log', 'message': f"Failed to send email to: {result['Email']}"})
                                else:
                                    message_queue.put({'type': 'log', 'message': f"Email template not found: {search_params['email_template']}"})
                        st.session_state.search_status = "completed"
                        message_queue.put({'type': 'log', 'message': f"Search completed. Total leads found: {len(st.session_state.search_results)}"})
                except Exception as e:
                    st.session_state.search_status = "failed"
                    message_queue.put({'type': 'log', 'message': f"Error during search: {str(e)}"})
        elif isinstance(message, dict) and message.get('type') == 'log':
            st.session_state.search_logs.append(message.get('message'))
        time.sleep(0.1)

# --- Add this function to save the background process state ---
def save_background_state():
    ...
    if SEARCH_PROCESS and SEARCH_PROCESS.is_alive():
        state = {
            ...
            'search_progress': st.session_state.get('search_progress', 0),
            'current_term': st.session_state.get('current_term', ''),
            'total_terms': st.session_state.get('total_terms', 0),
            'message_queue': list(MESSAGE_QUEUE.queue) if MESSAGE_QUEUE else []
        }
        with open('background_state.pkl', 'wb') as f:
            pickle.dump(state, f)

# --- Add this function to load the background process state ---
def load_background_state():
    global SEARCH_PROCESS, MESSAGE_QUEUE
    try:
        with open('background_state.pkl', 'rb') as f:
            state = pickle.load(f)
            st.session_state.search_status = state.get('search_status', 'idle')
            st.session_state.search_results = state.get('search_results', [])
            st.session_state.search_logs = state.get('search_logs', [])
            st.session_state.search_progress = state.get('search_progress', 0)
            st.session_state.current_term = state.get('current_term', '')
            st.session_state.total_terms = state.get('total_terms', 0)
            if MESSAGE_QUEUE:
                MESSAGE_QUEUE.queue.extend(state.get('message_queue', []))
...
def main():
    ...
    with st.sidebar:
        selected = option_menu(
            ...
            on_change=lambda: navigate_to(st.session_state.get('selected_option', "🔍 Manual Search")),
            key='selected_option'
        )
...
def fetch_search_term_analytics():
    with db_session() as session:
        query = (session.query(SearchTerm.term, func.count(Lead.id), func.count(EmailCampaign.id))
             .join(LeadSource, SearchTerm.id == LeadSource.search_term_id)
             .join(Lead, LeadSource.lead_id == Lead.id)
             .outerjoin(EmailCampaign, Lead.id == EmailCampaign.lead_id)
             .group_by(SearchTerm.term))
...
def search_terms_page():
    st.title("Search Terms Analytics")
    try:
        df = fetch_search_term_analytics()
        if not df.empty:
            st.dataframe(df)
            fig = px.bar(df, x='Term', y=['Lead Count', 'Email Count'], barmode='group', title='Search Term Analytics')
            st.plotly_chart(fig)
        else:
            st.info("No search term data available.")
    except Exception as e:
        st.error(f"An error occurred while fetching search term analytics: {str(e)}")
        logging.error(f"Error in search_terms_page: {str(e)}")
