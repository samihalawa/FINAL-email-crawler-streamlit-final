...
def view_sent_email_campaigns():
    ...
    refresh_button = st.button(
        "Refresh Campaigns",
        disabled=st.session_state.email_view_state['is_loading']
    )
    
    try:
        with db_session() as session:
            # Track loading state
            st.session_state.email_view_state['is_loading'] = True
            ...
            if time.time() - start_time > 30:  # Timeout after 30 seconds
                st.warning("Query taking longer than expected. Consider refining your search.")
            ...
            if not email_campaigns.empty:
                ...
                selected_campaign = st.selectbox(
                    ...
                    email_campaigns['ID'].tolist(),
                    key='campaign_selector'
                )
                
                if selected_campaign:
                    campaign_content = email_campaigns[
                        email_campaigns['ID'] == selected_campaign
                    ]['Content'].iloc[0]
                    
                    # Add content preview with resource limits
                    preview_length = min(len(campaign_content), 10000)  # Limit preview size
                    st.text_area(
                        "Content",
                        campaign_content[:preview_length] if campaign_content else "No content available",
                        height=300
                    )
            else:
        ...
        # Cleanup state
        st.session_state.email_view_state['is_loading'] = False
        st.session_state.email_view_state['last_refresh'] = time.time()
