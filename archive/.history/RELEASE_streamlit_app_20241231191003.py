...
def manual_search_page():
    ...
    with db_session() as session:
        campaign_id = get_active_campaign_id()
        if not campaign_id:
            st.error("No active campaign found. Please create a campaign first.")
            return
    ...
    for index, lead in enumerate(leads):
        ...
        try:
            ...
            if response:
                ...
                failed_emails[lead['Email']] = "Failed to send"
                log_message = f"❌ Failed to send email to: {lead['Email']}"
            
            save_email_campaign(session, lead['Email'], template_id, status, datetime.utcnow(), 
                              template.subject, message_id, template.body_content)
            ...
            failed_emails[lead['Email']] = "Invalid email"
            logs.append(log_message)
        except Exception as e:
            error_message = f"Error sending email to {lead['Email']}: {str(e)}"
            logging.error(error_message)
            failed_emails[lead['Email']] = str(e)
            save_email_campaign(session, lead['Email'], template_id, 'failed', datetime.utcnow(), 
                              template.subject, f"error-{uuid.uuid4()}", template.body_content)
            logs.append(f"❌ Error sending email to: {lead['Email']} (Error: {str(e)})")

    # Store updated sets back in session state
    st.session_state.sent_emails = sent_emails
    st.session_state.failed_emails = failed_emails

    # Show summary of failed emails
    if failed_emails and log_container:
...
def manual_search_page():
    ...
    if enable_email_sending:
        ...
        # Set these to None when email sending is disabled
        from_email = None
        reply_to = None
        email_template = None
