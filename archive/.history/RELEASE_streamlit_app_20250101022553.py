...
def manual_search_page():
    ...
    with st.form("search_form"):
        search_terms = st.text_area("Enter search terms (one per line)", height=100)
        num_results = st.number_input("Number of results per term", min_value=1, value=10)
        
        col1, col2 = st.columns(2)
        with col1:
            ignore_previously_fetched = st.checkbox("Ignore previously fetched URLs", value=True)
        ...
        with col2:
            ...
            if enable_email_sending:
                with db_session() as session:
                    ...
                    from_email = st.selectbox(
                        "From Email",
                        options=[f"{setting['name']} ({setting['email']})" for setting in email_settings],
                        format_func=lambda x: x.split('(')[1].rstrip(')')
                    )
                    reply_to = st.text_input("Reply-To Email", value=from_email.split('(')[1].rstrip(')') if from_email else "")
                    ...
                    email_template = st.selectbox(
                        "Email Template",
                        options=[f"{template.id}: {template.template_name}" for template in templates]
                    )
        ...
        with db_session() as session:
            ...
            search_process = SearchProcess(
                ...
                settings={
                    ...
                    'from_email': from_email.split('(')[1].rstrip(')') if enable_email_sending else None,
            ...
            session.commit()
            process_id = search_process.id
            
            if run_in_background:
                # Start background process
                thread = threading.Thread(
                    target=background_manual_search,
                    args=(process_id, search_terms, search_process.settings),
                    daemon=True
                )
                thread.start()
                
                st.success(f"Search process started in background! Process ID: {process_id}")
                
                # Create a container for live logs
                logs_expander = st.expander("Live Process Logs", expanded=True)
                with logs_expander:
                    st.markdown("""
                        <style>
                            .process-logs {
                    ...
                    while True:
                        with db_session() as session:
                            ...
                            if process.logs:
                                display_process_logs(process_id)
                                
                            if process.status in ['completed', 'failed']:
                                if process.status == 'completed':
                                    st.success(f"Search completed! Found {process.total_leads_found} leads.")
                                else:
                                    st.error(f"Search failed: {process.error_message}")
                                break
                ...
                results = manual_search(
                    ...
                    log_container,
                    from_email.split('(')[1].rstrip(')') if enable_email_sending else None,
                ...
                if results:
                    st.success(f"Search completed! Found {len(results.get('results', []))} leads.")
...
def bulk_send_emails(session, template_id, from_email, reply_to, leads, progress_bar=None, status_text=None, results=None, log_container=None):
    ...
    try:
        ...
        # Show summary
        if failed_emails:
            log_container.warning(f"Failed to send {len(failed_emails)} emails. You can retry failed sends.")
...
def background_manual_search(process_id, search_terms, settings):
    """Run manual search in background with proper logging"""
    try:
        with db_session() as session:
            process = session.query(SearchProcess).get(process_id)
            if not process:
            ...
            update_log(None, "Starting search process", 'info', process_id)
            ...
            results = manual_search(
                ...
                process_id  # Pass process_id for logging
            )
            
            if results:
                process.total_leads_found = len(results.get('results', []))
                process.results = results
                process.status = 'completed'
                process.completed_at = datetime.utcnow()
                update_log(None, f"Process completed. Found {process.total_leads_found} leads", 'success', process_id)
                session.commit()
        ...
        with db_session() as session:
            ...
            if process:
                ...
                process.completed_at = datetime.utcnow()
                update_log(None, f"Process failed: {str(e)}", 'error', process_id)
                session.commit()
