...
engine = create_engine(
    DATABASE_URL,
    pool_size=5,  # Reduce from 20 to 5
    max_overflow=2,  # Reduce from 0 to 2
...
def fetch_leads_with_sources(session):
    try:
        ...
        logging.info("Starting fetch_leads_with_sources")
        
        # Build the query with explicit joins and conditions
        query = (session.query(
            Lead,
        ...
        .outerjoin(LeadSource, Lead.id == LeadSource.lead_id)
        .outerjoin(EmailCampaign, Lead.id == EmailCampaign.lead_id)
        .group_by(Lead.id)
        .order_by(Lead.created_at.desc()))  # Order by creation date
        
        # Execute query and log result count
        results = query.all()
        ...
        return pd.DataFrame(columns=['id', 'email', 'first_name', 'last_name', 'company', 'job_title', 'created_at', 'Source', 'Last Contact', 'Last Email Status', 'Delete'])
    except Exception as e:
