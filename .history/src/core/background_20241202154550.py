...
from core.database import get_db, Project, SearchTermGroup, SearchTerm
...
class TaskManager:
    ...
    def __init__(self):
        ...
        self.group_queues: Dict[int, asyncio.Queue] = {}  # group_id -> task queue
        self.group_workers: Dict[int, List[asyncio.Task]] = {}  # group_id -> list of worker tasks
    ...
    async def stop(self):
        ...
        # Cancel all group workers
        for workers in self.group_workers.values():
            for worker in workers:
                if not worker.done():
                    worker.cancel()
    ...
    async def start_project_tasks(self, project_id: int):
        ...
        # Start group workers for each search term group
        async with get_db() as session:
            groups = await session.execute(
                "SELECT id FROM search_term_groups WHERE project_id = :project_id",
                {'project_id': project_id}
            )
            for group in groups.scalars().all():
                await self.start_group_tasks(group.id)
        
        # Schedule initial tasks
        await self.schedule_project_tasks(project_id)
    
    async def start_group_tasks(self, group_id: int):
        """Start tasks for a search term group"""
        if group_id in self.group_queues:
            return
        
        self.group_queues[group_id] = asyncio.Queue()
        
        # Start workers for this group
        self.group_workers[group_id] = [
            asyncio.create_task(self.group_worker(group_id))
            for _ in range(3)  # Number of workers per group
        ]
        
        # Schedule initial group tasks
        await self.schedule_group_tasks(group_id)
    ...
    async def stop_project_tasks(self, project_id: int):
        ...
        # Stop group workers for this project
        async with get_db() as session:
            groups = await session.execute(
                "SELECT id FROM search_term_groups WHERE project_id = :project_id",
                {'project_id': project_id}
            ...
            for group in groups.scalars().all():
                await self.stop_group_tasks(group.id)
        
        # Clear active tasks
        del self.active_tasks[project_id]
    
    async def stop_group_tasks(self, group_id: int):
        """Stop tasks for a search term group"""
        if group_id in self.group_workers:
            for worker in self.group_workers[group_id]:
                if not worker.done():
                    worker.cancel()
            del self.group_workers[group_id]
        
        if group_id in self.group_queues:
            while not self.group_queues[group_id].empty():
                try:
                    self.group_queues[group_id].get_nowait()
                except:
                    pass
    ...
    async def schedule_project_tasks(self, project_id: int):
        ...
        async with get_db() as session:
            ...
            # Schedule search tasks for ungrouped terms
            results = await session.execute(
                "SELECT term FROM search_terms WHERE project_id = :project_id AND group_id IS NULL",
                {'project_id': project_id}
            )
            ...
            # Schedule email tasks for pending leads
            results = await session.execute(
                "SELECT id FROM leads WHERE project_id = :project_id AND status = 'pending'",
                {'project_id': project_id}
            )
    ...
    async def schedule_group_tasks(self, group_id: int):
        """Schedule tasks for a search term group"""
        async with get_db() as session:
            group = await session.get(SearchTermGroup, group_id)
            if not group:
                return
            ...
            # Get search terms for this group
            results = await session.execute(
                "SELECT term FROM search_terms WHERE group_id = :group_id",
                {'group_id': group_id}
            )
            terms = results.scalars().all()
            
            # Schedule search tasks for this group
            for term in terms:
                await self.group_queues[group_id].put({
                    'type': 'search',
                    'data': {
                        'term': term,
                        'excluded_domains': project.settings['excluded_domains'],
    ...
    async def task_worker(self, project_id: int):
        """Worker process for executing project tasks"""
        while True:
            try:
                # Get task from queue
    ...
    async def group_worker(self, group_id: int):
        """Worker process for executing group tasks"""
        while True:
            try:
                # Get task from queue
                task = await self.group_queues[group_id].get()
                
                # Execute task
                if task['type'] == 'search':
                    await self.execute_group_search_task(group_id, task['data'])
                
                self.group_queues[group_id].task_done()
                
            except asyncio.CancelledError:
                break
            except Exception as e:
                app_logger.error(f"Group task error in group {group_id}", exc_info=e)
    ...
    async def execute_group_search_task(self, group_id: int, data: Dict[str, Any]):
        """Execute a search task for a group"""
        try:
            # Get optimized search strategy for this group
            async with get_db() as session:
                group = await session.get(SearchTermGroup, group_id)
                if not group:
                    return
                
                # Use AI to optimize the search term for this group
                optimized_term = await ai_service.optimize_group_search_term(
                    term=data['term'],
                    group_name=group.name
                )
                
                # Perform search with optimized term
                results = await search_service.search(
                    query=optimized_term,
                    excluded_domains=data['excluded_domains']
                )
                ...
                for result in results:
                    ...
                    existing = await session.execute(
                        "SELECT id FROM leads WHERE email = :email AND project_id = :project_id",
                        {'email': result['email'], 'project_id': group.project_id}
                    )
                    if existing.scalar():
                        continue
                    
                    # Create new lead with group association
                    lead = Lead(
            ...
            app_logger.error(f"Group search task error in group {group_id}", exc_info=e)
            ...
            try:
                ...
                for project_id in list(self.active_tasks.keys()):
                    async with get_db() as session:
                        ...
                        # Check bounce rate
                        if project.settings['automation']['auto_pause_on_bounce']:
                            bounce_rate = await self.calculate_bounce_rate(project_id)
                            if bounce_rate > project.settings['automation']['auto_pause_threshold']:
                ...
                # Monitor groups
                for group_id in list(self.group_queues.keys()):
                    if self.group_queues[group_id].empty():
                        await self.schedule_group_tasks(group_id)
    ...
    async def calculate_bounce_rate(self, project_id: int) -> float:
        """Calculate current bounce rate for a project"""
        async with get_db() as session:
            results = await session.execute(
                """
                SELECT 
