...
from sqlalchemy import func, create_engine, Column, BigInteger, Text, DateTime, ForeignKey, Boolean, JSON, Integer, String, Float
...
def cache_db_query(query_key: str, ttl: int = CACHE_TTL) -> Any:
    """Cache database query results."""
    pass  # Implementation depends on specific query

# --------------------- Utility Functions ---------------------
def safe_request(url: str, headers: Dict[str, str], timeout: int = REQUEST_TIMEOUT) -> Optional[requests.Response]:
...
def get_email_preview(session, template_id, lead_id=None):
    """Get a preview of an email template, optionally customized for a specific lead."""
    try:
        ...
        if lead_id:
            ...
            if lead:
                # Customize template with lead info
                preview['subject'] = preview['subject'].replace('{first_name}', lead.first_name or '')
                preview['subject'] = preview['subject'].replace('{last_name}', lead.last_name or '')
                preview['subject'] = preview['subject'].replace('{company}', lead.company or '')
                
                preview['body'] = preview['body'].replace('{first_name}', lead.first_name or '')
                preview['body'] = preview['body'].replace('{last_name}', lead.last_name or '')
                preview['body'] = preview['body'].replace('{company}', lead.company or '')

        return preview

    except SQLAlchemyError as e:
        logger.error(f"Database error in get_email_preview: {str(e)}")
        return None
    except Exception as e:
        logger.error(f"Error in get_email_preview: {str(e)}")
        return None
...
def bulk_send_page():
        ...
        with st.form("bulk_send_form"):
            ...
            with col1:
                with get_db() as session:
                    ...
                    templates = [(t.id, t.template_name) for t in session.query(EmailTemplate).all()]
                    if not templates:
                        st.error("No email templates found. Please create a template first.")
                        return
                    template = st.selectbox(
                        "Select Email Template",
                        options=[f"{id}:{name}" for id, name in templates]
                    )
            ...
            with col2:
                send_option = st.radio(
                    "Send to:",
                    ["All Leads", "All Not Contacted with this Template", 
                     "All Not Contacted with Templates from this Campaign",
                     "Leads from Selected Search Terms", 
                     "Leads from Selected Search Term Groups"]
                )
                ...
                if send_option in ["Leads from Selected Search Terms", "Leads from Selected Search Term Groups"]:
                    with get_db() as session:
                        if send_option == "Leads from Selected Search Terms":
                            # Get search terms with lead count
                            query = session.query(
                                SearchTerm.id,
                                SearchTerm.term,
                                func.count(LeadSource.id).label('lead_count')
                            ).outerjoin(LeadSource).group_by(SearchTerm.id, SearchTerm.term)
                            terms = query.all()
                            selected_items = st.multiselect(
                                "Select Search Terms",
                                options=[f"{t.id}: {t.term} ({t.lead_count} leads)" for t in terms]
                            )
                        else:
                            # Get search term groups
                            groups = session.query(SearchTermGroup.id, SearchTermGroup.name).all()
                            selected_items = st.multiselect(
            ...
            send_button = st.form_submit_button("Start Bulk Send")
    ...
    if st.session_state.get('bulk_send_in_progress', False):
        ...
        with get_db() as session:
            ...
            # Get leads based on send option
            if params['send_option'] == "Leads from Selected Search Terms":
                search_term_ids = [int(term.split(":")[0]) for term in params['selected_items']]
                query = session.query(Lead).distinct().\
                    join(LeadSource).\
                    filter(LeadSource.search_term_id.in_(search_term_ids))
                
            elif params['send_option'] == "Leads from Selected Search Term Groups":
                group_ids = [int(group.split(":")[0]) for group in params['selected_items']]
                query = session.query(Lead).distinct().\
                    join(LeadSource).\
                    join(SearchTerm).\
                    filter(SearchTerm.group_id.in_(group_ids))
                
            elif params['send_option'] == "All Not Contacted with this Template":
                subquery = session.query(EmailCampaign.lead_id).\
                    filter(EmailCampaign.template_id == params['template_id'])
                query = session.query(Lead).filter(~Lead.id.in_(subquery))
                
            else:
                query = session.query(Lead)

            # Apply blog-directory filter if selected
            if params['filter_option'] == "Filter Out blog-directory":
                query = query.filter(
                    ~Lead.id.in_(
                        session.query(Lead.id).\
                        join(LeadSource).\
                        filter(LeadSource.tags.contains('blog-directory'))
                    )
                )

            leads_to_send = [(lead.id, lead.email) for lead in query.all()]
            ...
            while st.session_state.current_lead_index < total_leads:
                ...
                try:
                    # Send email
                    template = session.query(EmailTemplate).get(params['template_id'])
                    if template:
                        ses_response = send_email_ses(
                            params['from_email'],
                            lead[1],
                            template.subject,
                        ...
                        if ses_response:
                            ...
                            st.session_state.bulk_send_results.append({
                                "Email": lead[1],
                                "Status": "sent",
                                "Message ID": ses_response['MessageId']
                            })
                ...
                with stats_container:
                    ...
                    success_rate = sum(1 for r in st.session_state.bulk_send_results if r['Status'] == 'sent') / len(st.session_state.bulk_send_results) * 100
    ...
    if not st.session_state.get('bulk_send_in_progress', False) and st.session_state.get('bulk_send_results'):
        ...
        df = pd.DataFrame(st.session_state.bulk_send_results)
        st.dataframe(df)
...
def fetch_grouped_search_terms(session):
    try:
        ...
        logger.error(f"Error in fetch_grouped_search_terms: {str(e)}")
        return {}
...
def fetch_search_terms_sorted(session):
    try:
        ...
        logger.info("Starting fetch_search_terms_sorted")

        query = (
            session.query(
                SearchTerm.id,
                SearchTerm.term,
                SearchTerm.effectiveness_score
            )
            .order_by(SearchTerm.effectiveness_score.desc())
            .all()
        )
        
        return query
    except Exception as e:
        logger.error(f"Error in fetch_search_terms_sorted: {e}")
        return []

def add_search_term(session, search_term, group=None):
    try:
        logger.info(f"Adding search term: {search_term}, group: {group}")
        new_term = SearchTerm(term=search_term, group=group)
        session.add(new_term)
        session.commit()
        logger.info(f"Successfully added search term: {search_term}")
    except Exception as e:
        logger.error(f"Error in add_search_term: {str(e)}")
        session.rollback()
        raise

def fetch_search_term_groups(session):
    try:
        logger.info("Fetching search term groups")
        query = session.query(func.coalesce(SearchTerm.group, 'Ungrouped')).distinct()
        groups = [group[0] for group in query.all()]
        logger.info(f"Fetched groups: {groups}")
        return groups
    except Exception as e:
        logger.error(f"Error in fetch_search_term_groups: {str(e)}")
        return []
...
def fetch_leads_for_search_term_groups(session, groups):
    try:
        ...
        logger.error(f"Error in fetch_leads_for_search_term_groups: {str(e)}")
        return []
...
def main():
    ...
    with st.sidebar:
        selected = option_menu(
            ...
            icons=["search", "send", "people", "key", "envelope", "graph-up", 
                  "folder", "book", "robot", "gear"],
            menu_icon="cast",
            default_index=0
        )
...
def handle_error(func):
    """Decorator for handling errors in page functions."""
    def wrapper(*args, **kwargs):
        try:
            ...
            st.error("An error occurred while processing your request with AI. Please try again later.")
        except requests.RequestException as e:
...
def display_metrics(
    ...
    with col2:
        st.metric("New Leads (24h)", new_leads_24h)
    with col3:
...
def display_success_message(
    ...
    """Display a success message that automatically disappears."""
...
if __name__ == "__main__":
    try:
        ...
        st.error("A fatal error occurred. Please contact support if the problem persists.")
