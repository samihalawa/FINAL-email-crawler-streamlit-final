...
class BackgroundProcessManager:
    ...
    def start_process(self, process_id, target, args):
        with self._lock:
            ...
            future = self._executor.submit(wrapped_target, *args)
    ...
    def stop_process(self, process_id):
        with self._lock:
            if process_id in self._processes:
                ...
                if not process_info['future'].done():
    ...
    def get_process_status(self, process_id):
        with self._lock:
            if process_id not in self._processes:
                return 'not_found'
            process_info = self._processes[process_id]
            if process_info['future'].done():
                return 'completed' if not process_info['future'].exception() else 'failed'
            return 'running'
...
# Update the background manual search function
def background_manual_search(process_id, search_terms, settings):
    """Execute manual search in background with proper process management"""
    try:
        with get_db() as session:
            results = manual_search(
                session,
        ...
        logger.error(f"Background search failed: {str(e)}")
        raise

# Update the manual search page
def manual_search_page():
    st.title("Manual Search")
    ...
    with db_session() as session:
        ...
        active_processes = session.query(SearchProcess).filter(
            SearchProcess.status.in_(['running', 'pending', 'completed'])
        ).order_by(SearchProcess.created_at.desc()).all()
            ...
            for process in active_processes:
                with st.expander(f"Process {process.id} - {process.status.title()} - Started at {process.created_at.strftime('%Y-%m-%d %H:%M:%S')}", expanded=True):
                    ...
                    if process.status == 'running':
                        if st.button(f"Stop Process {process.id}", key=f"stop_{process.id}"):
                            if background_process_manager.stop_process(process.id):
                                st.success(f"Process {process.id} stopped")
                                st.rerun()
        ...
        if st.button("Start Search", type="primary", use_container_width=True):
            ...
            if run_in_background:
                if background_process_manager.start_process(
                    search_process.id,
                    background_manual_search,
                    (search_process.id, search_terms, search_process.settings)
                ):
                    st.success(f"Search process started in background! Process ID: {search_process.id}")
                else:
...
def cleanup_on_shutdown():
    background_process_manager.shutdown()
