...
engine = create_engine(
    DATABASE_URL,
    pool_size=1,  # Minimal pool size to strictly limit connections
    max_overflow=1,  # Minimal overflow to prevent connection issues
    pool_timeout=20,  # Shorter timeout to fail fast if no connections available
    pool_recycle=180,  # More aggressive connection recycling (3 minutes)
    pool_pre_ping=True,  # Keep pre-ping enabled to detect stale connections
    echo=False,
